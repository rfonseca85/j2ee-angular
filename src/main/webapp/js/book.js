...
bookControllers.controller('bookCtrl', function ($scope, $rootScope, bookService) {
    ...
    $scope.gridOptions = {
        ...
        afterSelectionChange: function (rowItem) {
            if (rowItem.selected) {
                $rootScope.$broadcast('bookSelected', $scope.gridOptions.selectedItems[0].id);
            }
        }
    };
    ...
    $scope.refreshGrid = function () {
        var listBooksArgs = {
            page: $scope.books.currentPage,
            sortFields: $scope.sortInfo.fields[0],
            sortDirections: $scope.sortInfo.directions[0]
        };

        bookService.get(listBooksArgs, function (data) {
            $scope.books = data;
        })
    };
    ...
    $scope.deleteRow = function (row) {
        $rootScope.$broadcast('deleteBook', row.entity.id);
    };
    ...
    // Picks the event broadcasted when a book is saved or deleted to refresh the grid elements with the most
    // updated information.
    ...
    // Calls the rest method to save a book.
    ...
    // Picks up the event broadcasted when the book is selected from the grid and perform the book load by calling
    // the appropiate rest service.
    $scope.$on('bookSelected', function (event, id) {
        $scope.book = bookService.get({id: id});
    });

    // Picks us the event broadcasted when the book is deleted from the grid and perform the actual book delete by
    // calling the appropiate rest service.
    $scope.$on('deleteBook', function (event, id) {
        bookService.delete({id: id}).$promise.then(
            function () {
                // Broadcast the event to refresh the grid.
                $rootScope.$broadcast('refreshGrid');
